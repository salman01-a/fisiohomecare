...
const createRecord = async (req, res, next) => {
  try {
    ...
    const currentSession = session_number || 1;
    ...
    // PERBAIKAN: Cek duplikasi berdasarkan order_id DAN session_number
    const existing = await TherapyRecord.findOne({ 
      where: { 
        order_id: order_id,
    ...
    if (existing) {
      throw ApiError.conflict(`Record already exists for this order at session ${currentSession}`);
    }
    ...
    await FirestoreService.sendPatientNotification(
      ...
      `Sesi terapi ${currentSession} telah selesai. Rekam medis telah diupdate.`,
      'success'
    );
